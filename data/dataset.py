...
def get_dataloader(pde_name,ntrain=1000,ntest=200,batch_size=64,seed=42,noise='None'):
...
# 'Circle_Darcy'
    ...
    elif pde_name.endswith('Izhikevich'):
        
        filepath='../data_folder/Izhikevich.npz'
    ...
    elif pde_name.endswith('Izhikevich_wavelet'):
        
        filepath='../data_folder/Izhikevich_wavelet.npz'
        data=np.load(filepath)
        if pde_name.startswith('inverse'):
            labels=data['I_ext']
            features=data['V']
    ...
    elif pde_name.endswith('BBP'):
        
        filepath='../data_folder/BBP_data.npz'
        data=np.load(filepath)
        if pde_name.startswith('inverse'):
            labels=data['I']
            features=data['V']
            ...
            features=data['I']
            labels=data['V']
        grid=np.linspace(0,2999,features.shape[1]).reshape(1,-1)
        grids=np.repeat(grid,features.shape[0],axis=0)
        features,labels,grids=torch.from_numpy(features).float(),torch.from_numpy(labels).float(),torch.from_numpy(grids).float()
