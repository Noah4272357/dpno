...
class FNO1d(nn.Module):
    def __init__(
        ...
        self.fc0 = nn.Sequential(
            nn.Linear(in_channels, 32),  # input channel is 2: (a(x), x)
            nn.GELU(),
            nn.Linear(32, self.width),
        )
        self.blocks = nn.ModuleList(
            [FNOBlock1d(self.modes, self.width) for _ in range(self.num_blocks)]
        )
...
class FNO2d(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [FNOBlock2d(self.modes1, self.modes2, width) for _ in range(num_blocks)]
        )
...
class FNO3d(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                FNOBlock3d(self.modes1, self.modes2, self.modes3, width)
...
class DNFNO2d(nn.Module):
    ...
    def forward(self, x, grid_y):
        ...
        x = F.pad(
            x, [0, self.padding, 0, self.padding]
