...
dataloader = scio.loadmat('/data/wyc/data/burgers.mat')

x_data = dataloader['input']
y_data = dataloader['output'][:,-1,:]

x_train =torch.from_numpy( x_data[:ntrain,:]).float()
y_train =torch.from_numpy( y_data[:ntrain,:]).float()
x_test = torch.from_numpy(x_data[-ntest:,:]).float()
y_test = torch.from_numpy(y_data[-ntest:,:]).float()
