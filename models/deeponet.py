...
from .model_utils import MLP, _get_act, _get_initializer


def get_model(pde_name, model_name, **args):
    if model_name == "DPDeepONet":
        if pde_name == "Burgers" or pde_name.endswith("Darcy"):
            model = DPDON1D(size=256, query_dim=2)
        elif pde_name == "Darcy_Flow":
            model = DPDON2D(size=128, query_dim=2)
        elif pde_name == "Navier_Stokes_2D" or pde_name == "Irregular_NS":
            model = DPDON3D(size=64, query_dim=3, time_step=10)
    elif model_name == "DeepONet":
        if pde_name == "Burgers":
            model = DeepONet1D(size=256, query_dim=2)
        elif pde_name == "HH":
            model = DeepONet1D(size=1001, query_dim=1)
        elif pde_name == "Darcy_Flow":
            model = DeepONet2D(size=128, query_dim=2)
        elif pde_name == "Navier_Stokes_2D":
            model = DeepONet3D(size=64, query_dim=3, time_step=10)
    return model
...
class DPDON1D(nn.Module):
    ...
    def __init__(
        ...
        self.branch = MLP(
            in_channel_branch,
            out_channel_branch + out_channel_trunk,
            layer_sizes,
            activation_branch,
            kernel_initializer,
        )
