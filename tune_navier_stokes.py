...
def get_arguments(parser):
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-4,
        help="Specifies learning rate for tuning. (default: 1e-6)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=5000,
        help="Number of training iterations. (default: 100k)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Save model checkpoints every x iterations. (default: 5k)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Channel of input feature. (default: 3)",
    )
    parser.add_argument(
        "--encoder_emb_dim",
        type=int,
        default=128,
        help="Channel of token embedding in encoder. (default: 128)",
    )
    parser.add_argument(
        "--out_seq_emb_dim",
        type=int,
        default=128,
        help="Channel of output feature map. (default: 128)",
    )
    parser.add_argument(
        "--encoder_depth",
        type=int,
        default=2,
        help="Depth of transformer in encoder. (default: 2)",
    )
    parser.add_argument(
        "--encoder_heads",
        type=int,
        default=4,
        help="Heads of transformer in encoder. (default: 4)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Channel of token embedding in decoder. (default: 128)",
    )
    parser.add_argument(
        "--out_step",
        type=int,
        default=10,
        help="How many steps to propagate forward each call. (default: 10)",
    )
    parser.add_argument(
        "--out_seq_len",
        type=int,
        default=40,
        help="Length of output sequence. (default: 40)",
    )
    parser.add_argument(
        "--propagator_depth",
        type=int,
        default=2,
        help="Depth of mlp in propagator. (default: 2)",
    )
    parser.add_argument(
        "--decoding_depth",
        type=int,
        default=2,
        help="Depth of decoding network in the decoder. (default: 2)",
    )
    parser.add_argument(
        "--fourier_frequency",
        type=int,
        default=8,
        help="Fourier feature frequency. (default: 8)",
    )
    parser.add_argument(
        "--use_grad",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="at initial stage, dont rollout too long",
    )
    parser.add_argument(
        "--curriculum_ratio",
        type=float,
        default=0.2,
        help="how long is the initial stage?",
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    y_train = data[
        opt.in_seq_len : opt.in_seq_len + opt.out_seq_len, ..., :ntrain
    ]  # solution: u(x)

    x_test = data[: opt.in_seq_len, ..., -ntest:]  # input: a(x)
    y_test = data[
        opt.in_seq_len : opt.in_seq_len + opt.out_seq_len, ..., -ntest:
    ]  # solution: u(x)
    ...
    grid = (
        rearrange(torch.from_numpy(xs), "c h w -> (h w) c").unsqueeze(0).float()
    )  # [64*64, 2]

    train_dataloader = DataLoader(
        TensorDataset(x_train, y_train), batch_size=opt.batch_size, shuffle=True
    )
    test_dataloader = DataLoader(
        TensorDataset(x_test, y_test), batch_size=opt.batch_size, shuffle=False
    )
    ...
    with tqdm(total=opt.iters) as pbar:
        ...
        while True and (not opt.eval_mode):
            ...
            if opt.curriculum_steps > 0 and n_iter < int(
                opt.curriculum_ratio * opt.iters
            ):
                progress = (n_iter * 2) / (opt.iters * opt.curriculum_ratio)
                curriculum_steps = (
                    opt.curriculum_steps
                    + int(
                        max(0, progress - 1.0)
                        * ((opt.out_seq_len - opt.curriculum_steps) / 2.0)
                    )
                    * 2
                )
            ...
            if opt.eval_mode or (
                (n_iter - 1) % opt.ckpt_every == 0 or n_iter >= opt.iters
            ):
                logger.info("Tesing")
                print("Testing")
                ...
                with torch.no_grad():
                    ...
                    for j, data in enumerate(tqdm(test_dataloader)):
                        ...
                        input_pos = prop_pos = repeat(
                            grid, "() n c -> b n c", b=in_seq.shape[0]
                        )
                        ...
                        x_out = decoder.rollout(
                            z, prop_pos, opt.out_seq_len, input_pos
                        )  # [b, seq_len, n]

                        x_out = x_out * y_std + y_mean  # denormalize

                        avg_loss = rel_loss(x_out, gt, p=2)
                        accumulated_mse = torch.nn.MSELoss(reduction="sum")(
                            x_out, gt
                        ) / (gt.shape[-1] * gt.shape[0])

                        loss_at_last_step = rel_loss(
                            x_out[:, -1:, ...], gt[:, -1:, ...], p=2
                        )
                        ...
                        in_seq = rearrange(
                            rearrange(in_seq, "b n t -> b t n") * x_std + x_mean,
                            "b t (h w) -> b t h w",
                            h=64,
                            w=64,
                        )
                ...
                gt = torch.cat(
                    (all_in_seq, all_gt), dim=1
                )  # concatenate in the temporal dimension
                pred = torch.cat(
                    (all_in_seq, all_pred), dim=1
                )  # concatenate in the temporal dimension
                ...
                writer.add_scalar(
                    "testing avg loss", np.mean(all_avg_loss), global_step=n_iter
                )
