...
from models.decoder_module import PointWiseDecoder1D
from models.encoder_module import Encoder1D
from training.losses import rel_loss
...
def build_model(res) -> (Encoder1D, PointWiseDecoder1D):
    ...
    decoder = PointWiseDecoder1D(96, 1, 3, scale=2, res=res)
...
def make_image_grid(
    init: torch.Tensor, sequence: torch.Tensor, gt: torch.Tensor, out_path, nrow=8
):
...
def pad_pbc(x: torch.Tensor, pos: torch.Tensor, h, pad_ratio=1 / 128):
    ...
    pos_pad_1 = 1 + offset
    pos_pad_0 = rearrange(
        torch.as_tensor(pos_pad_0).to(pos.device), "n -> 1 n 1"
    ).repeat([x.shape[0], 1, 1])
    pos_pad_1 = rearrange(
        torch.as_tensor(pos_pad_1).to(pos.device), "n -> 1 n 1"
    ).repeat([x.shape[0], 1, 1])
    return torch.cat([pad_0, x, pad_1], dim=1), torch.cat(
        [pos_pad_0, pos, pos_pad_1], dim=1
    )
...
def get_arguments(parser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=2048,
        help="The interval of when sample snapshots from sequence",
    )
...
if __name__ == "__main__":
    ...
    with tqdm(total=opt.iters) as pbar:
        ...
        while True:
            ...
            x = torch.cat(
                (x, input_pos), dim=-1
            )  # concat coordinates as additional feature
            ...
            if (n_iter - 1) % opt.ckpt_every == 0 or n_iter >= opt.iters:
                ...
                save_checkpoint(
                    ckpt, os.path.join(checkpoint_dir, f"model_checkpoint{n_iter}.ckpt")
                )
                del ckpt
                if n_iter >= opt.iters:
                    break
